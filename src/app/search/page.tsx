...
export default async function SearchPage({ searchParams } : { searchParams : { [key: string]: string | string[] | undefined } }) {
  ...
  const [products, relatedWords] = await Promise.all([productsData, relatedWordsData])
  ...
  return (
    <div className="grid mt-4 gap-4">
      ...
      {relatedWords.data.length > 0 ? (<ProductRelated relatedWords={relatedWords.data}/>) : (<div className="w-full h-2"></div>)}
