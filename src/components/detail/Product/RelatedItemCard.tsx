...
export default function RelatedItemCard({ product }: RelatedItemCardProps) {
  ...
  const handleCardClick = () => {
    router.push(`/detail/${product.auctionId}`); // 이 경로는 실제 라우팅 구조에 맞게 수정
  };

  return (
    <div
      onClick={handleCardClick}
      className="p-2 rounded-xl shadow-none w-[231px] bg-white"
    >
      <div className="grid grid-cols-1 grid-rows-1">
        <img
          src={
            `http://localhost:8080${product.thumbnailUrl}` || "/placeholder.jpg"
          }
          alt={product.title}
          width={231}
          height={231}
      ...
      <div className="pt-3 space-y-1">
        ...
        <div className="flex items-center gap-1 text-xs text-gray-600">
          <span>🕒 종료 :</span>
          <span>
            {new Date(product.endTime).toLocaleDateString("ko-KR", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
