...
import { bidEndInfo,getPurchaseStatus } from "./constants/MyPageProduct";

export default function ProductPurchasedCard(){
  const status = getPurchaseStatus(bidEndInfo.ispurchased);
  return(
    <div className="flex justify-center w-full">
      ...
      <div className="flex items-center h-full">
          ...
          <div className="flex-1">
            <p className="font-bold text-[16px]">{bidEndInfo.productName}</p>
            ...
            <div className="flex justify-between text-[16px]">
              <span className="w-1/2">{bidEndInfo.myBid}</span>
            </div>

            <p className="text-xs mt-6">주문 번호: {bidEndInfo.orderNum}</p>
          </div>
